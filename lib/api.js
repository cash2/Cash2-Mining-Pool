...
function getNetworkData(callback, rpcMode) {
    ...
    if (rpcMode === 'get_info') {
        apiInterfaces.rpcDaemon('get_info', {}, function(error, reply){
            if (error) {
                getNetworkData(callback, 'get_last_block_header');
                return;
            } else {
                networkDataRpcMode = 'get_info';

                var diff = reply.difficulty;

                if (reply.height >= CASH2_HARD_FORK_HEIGHT_2)
                {
                  diff = bignum('1099511627776').mul(reply.difficulty).toString();
                }
    ...
    // Else fallback to getlastblockheader
        ...
        apiInterfaces.rpcDaemon('get_last_block_header', {}, function(error, reply){
            if (error){
                ...
                networkDataRpcMode = 'get_last_block_header';

                var blockHeader = reply.block_header;

                var diff = blockHeader.difficulty;

                if (blockHeader.height + 1 >= CASH2_HARD_FORK_HEIGHT_2)
                {
                  diff = bignum('1099511627776').mul(blockHeader.difficulty).toString();
                }
...
function handleSetTelegramNotifications(urlParts, response){
    ...
    // Check action
    if ((type !== 'default' || type !== 'cash2') && (action === undefined || action === '' || (action != 'enable' && action != 'disable'))) {
        response.end(JSON.stringify({status: 'Invalid action'}));
        return;
    }
...
// Start Daemon RPC monitoring
function startDaemonRpcMonitoring(rpcDaemon, module, method, interval) {
    setInterval(function() {
        rpcDaemon(method, {}, function(error, response) {
            var stat = {
...
// Start Wallet RPC monitoring
function startWalletRpcMonitoring(rpcWallet, module, method, interval) {
    setInterval(function() {
        rpcWallet(method, {}, function(error, response) {
            var stat = {
                lastCheck: new Date() / 1000 | 0,
                lastStatus: error ? 'fail' : 'ok',
                lastResponse: JSON.stringify(error ? error : response)
...
function initMonitoring() {
    ...
    for(var module in config.monitoring) {
        ...
        if ((daemonType === "bytecoin" || daemonType === "cash2") && module === "wallet")
        {
          settings.rpcMethod = "get_balance";
        
          if(settings.checkInterval) {
              startWalletRpcMonitoring(modulesRpc[module], module, settings.rpcMethod, settings.checkInterval);
          }
        }
        else
        {
          settings.rpcMethod = "get_info";
        
          if(settings.checkInterval) {
              startDaemonRpcMonitoring(modulesRpc[module], module, settings.rpcMethod, settings.checkInterval);
          }
        }
    }
}

...
/**
 * Get block template using rpc call to cash2 daemon
 **/
...
function processBlockTemplate(template) {
  ...
  // broadcast new block to all connected miners
  for (var minerId in connectedMiners){
    var miner = connectedMiners[minerId];
    
    // getJob() loads the currentBlockTemplate into each Miner object
    var job = miner.getJob();

    if (miner.socket && miner.socket.readyState == "open")
    {
      var sendData = JSON.stringify({
        params:
        [
...
function jobRefresh(loop, callback){
  ...
  // call get block template rpc command
  getBlockTemplate(function(error, result){
    ...
    if (error)
    {
      log('error', logSystem, 'Error calling getblocktemplate %j', [error]);
    ...
    // start pool after getting a block from cash2 daemon
    if (!poolStarted)
    {
 ...
 * Handle miner method
 **/
function handleMinerMethod(method, params, socket, portData, sendReply, id)
{
  ...
  // console.log("number of connected miners = " + Object.keys(connectedMiners).length);

  // Check for ban here, so preconnected attackers can't continue to screw you

  // console.log('\x1b[36m', 'socket.remoteAddress = ' + socket.remoteAddress ,'\x1b[0m');
  // console.log('\x1b[36m', 'socket.remoteAddress == "::ffff:85.135.134.104" ' + (socket.remoteAddress == "::ffff:85.135.134.104") ,'\x1b[0m');
  ...
  switch(method){
    ...
    case 'mining.authorize':
      ...
      // console.log('\x1b[36m', 'mining.authorize params = ' + params ,'\x1b[0m');
      ...
      // console.log("mining.authorize miner = " + JSON.stringify(miner));
    ...
    case 'mining.submit':
      ...
      if (!job)
      {
        ...
        socket.write(sendData);
        console.log('\x1b[36m', 'job not found for ip = ' + miner.ip ,'\x1b[0m');
        console.log('\x1b[36m', 'jobId = ' + jobId + ", job.id = " + job.id ,'\x1b[0m');
...
function startPoolServerTcp(callback)
{
  ...
  async.each(config.poolServer.ports, function(portData, cback) {
    var handleMessage = function(socket, jsonData) {
      ...
      // socketsGlobal.add(socket);

      // console.log("socketsGlobal.size = " + socketsGlobal.size);
      ...
      handleMinerMethod(jsonData.method, jsonData.params, socket, portData, sendReply, jsonData.id);
    };

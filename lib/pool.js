...
function BlockTemplate(template){
  ...
  this.previousBlockHash = template.blocktemplate_blob.substr(0, 64);

  // timestamp is 8 bytes long or 16 hex characters
  this.timestamp = template.blocktemplate_blob.substr(80, 16);
  ...
  this.coinbaseTransaction1 = template.coinbase_tx.substr(coinbaseEnd - 240, (2 * template.reserved_offset - 160 - (coinbaseEnd - 240 + 2)));
  
  // 2 x reserve_offset - 160 - 2 gives the start of extra nonce 1.
  // 16 is the size of extra nonce 1 plus extra nonce 2
  this.coinbaseTransaction2 = template.coinbase_tx.substr(2 * template.reserved_offset - 160 - 2 + 16);
...
function jobRefresh(loop , callback)
{
  ...
  getBlockCount(function(error0, result0) {
    ...
    if (error0)
    {
      log('error', logSystem, 'Error calling getblockcount %j', [error]);

    ...
    async.waterfall([
        ...
        function(blocks, callback){
            ...
            async.filter(blocks, function(block, mapCback){
                ...
                apiInterfaces.rpcDaemon('get_block_header_by_height', {height: blockHeight}, function(error, result){
                    ...
                    if (error){
                        log('error', logSystem, 'Error with get_block_header_by_height RPC request for block %s - %j', [block.serialized, error]);
                        block.unlocked = false;
                        mapCback();
                        return;
                    }
                    if (!result.block_header){
                        log('error', logSystem, 'Error with get_block_header_by_height, no details returned for %s - %j', [block.serialized, result]);
                        block.unlocked = false;
                        mapCback();
                        return;
                    }
